...
setwd("D:/Users/uusitalol/RStudio/2019 BLUEWEBS FCM")
...
ggplot(mind, aes(x=Variables, y=Density, shape=Group, color=Group)) +
  geom_point(size=4) #+  geom_rug() 
  #geom_smooth(method=lm), se=FALSE, fullrange=TRUE)

#correlation between #variables and conn.density =~ -0.08
cor(mind$Variables, mind$Density)

pnew <- ggplot(mind, aes(x=Variables, y=Density, shape=Group, color=Group)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), 
        legend.position = "right") + 
  geom_point(size=2) + labs(title="Number of variables and connection density of each map", x="Variables", y="Connection density")



png(file="VarNo-Density2.png", units="cm", width = 20, height = 20, res=200)
grid.arrange(
  grobs = list(p1, p2, pnew),
...
cenRec<-ggplot(centralitiesRec, aes(x=Concept, y=Centrality, fill=Concept)) + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), 
        axis.text.x = element_text(angle = 90, hjust = 1), axis.title.x = element_blank(), legend.position = "none") + 
  labs(y="Variable centrality", xlab=" ") + annotate("text", x=1, y=13, label="e) Recreation, n=6", hjust=0) + labs(title=element_blank())
...
png(file="centralityplot2.png", units="cm", width = 32, height = 30, res=200)
...
malli.mds$stress #stress: 11.96971

#three-dimensional
malli3.mds <- isoMDS(d, k=3)
MDS_xy3 <- data.frame(malli3.mds$points)
MDS_xy3$Group <-  Group
group <- as.factor(MDS_xy3$Group)
malli3.mds$stress # stress: 8.20384
...
MDS_xy2$Group <-  Group
malli2.mds$stress # stress: 18.67582

#3d model
malli23.mds <- isoMDS(d2, k=3)
MDS_xy23 <- data.frame(malli23.mds$points)
MDS_xy23$Group <-  Group
malli23.mds$stress # stress: 13.28691
...
ClustLinkages <- ggplot(MDS_xy2, aes(x=X1, y=X2, shape=Group, color=Group)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), 
        axis.text.x = element_blank(), axis.title.x = element_blank(), legend.position = "right") + 
...
#there's probably a function that doe s this prettily...
...
ggarrange(allfPp2, allePp2, alltPp2, allsPp2, 
          allfMp2, alleMp2, alltMp2, allsMp2, ncol = 4)
dev.off()
...
ggarrange(allfMp2, fisfMp2, engofMp2,
          recfMp2, polfMp2, scifMp2, ncol = 3)
dev.off()
...
ggarrange(alleMp2, fiseMp2, engoeMp2,
          receMp2, poleMp2, scieMp2, ncol = 3)
dev.off()
